...
std::ofstream myfile("/home/bros/pose.csv",std::ios::out); //change this accordingly
...
void dataCB(const geometry_msgs::Vector3::ConstPtr& dat)
{
	...
	if(flag_first == 0 and *left_reference == 1 and *right_reference == 1)
	{
		...
		{
			ROS_ERROR(" POSE : Encoder ppr not received: Set parameter /left_enc_ppr and /right_enc_ppr");
		}
        else
        {
            CIRC = 0.07068583462;
...
int main(int argc,char **argv)
{
	...
	ros::NodeHandle n;
	ros::Publisher cmd_pub = n.advertise<geometry_msgs::Twist>("/pose_dir", 10);
	...
	ros::Subscriber get_pose=n.subscribe<std_msgs::Int8>("/pose_starting",10,&pose_start_callback);
	...
	while (ros::ok()) 
	{
		if(flag_first == 2)
		{
			...
			if(p_data == 1 and first_time == 1)
			{
				...
				msg1.linear.z = float(new_degree);
				msg1.angular.z = float(dir_degree);

				msg1.angular.y = float(zo);

				msg1.angular.x = float(ss_degree);
