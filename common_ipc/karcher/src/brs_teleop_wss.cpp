...
void alarm_wss(const std_msgs::Int8::ConstPtr& al)
{
	wss_alarm = al->data;
}
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber alarm_wss_ = n.subscribe<std_msgs::Int8>("/wss_alarm",10,&alarm_wss);
	...
	while (ros::ok())
	{
        ...
        else if(wss_alarm == 96)
        {
            c = 'b';
            wss_alarm = 0;
        }
        else if(wss_alarm == 97)
        {
            c = 'h';
            wss_alarm = 0;
        }
		...
		if(c>0 and c!= -1)
        {
            ...
            {
                ROS_INFO("Teleop : P to P set at 20.0 feet ");
                ros::param::set("/p_to_p",20.0); //second lane
                ros::param::set("/new_command",1);
