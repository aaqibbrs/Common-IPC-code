...
float setpoint_array[100], lg[100], err, ws_setpoint, lidar_end;
	...
	if(allowed == 1)
	{
		...
		if(starter == 10)
		{
			...
			for(int i = 0; i < count; i++)
			{
				...
				else
				{
					...
					if((degree>=30.0 and degree<=89.0) and (scan->ranges[i] < (2.0 * setpoint_array[int(degree)])) and scan->ranges[i]>0.1 and scan->ranges[i]<6.0 and scan->ranges[i] < lg[int(degree)] and scan->intensities[i]!=0)
					{
			...
			if(z > 3 and first == 2)
			{
					...
					if((flag_sf == 0 or flag_sf == 2) and reference != 1)
					{
						...
						if(p<presence and done_pp != 2)
						{
							if(done_pp == 0)
							{
								first_pp = left_wheel;
								done_pp = 1;
								ROS_INFO("LWF : P to P started with P to P as %f", p_to_p);
							}
							if(done_pp == 1)
							{
								...
								if(dist_pp>=enc_pp)
								{
									done_pp = 2;
									flag_turn = 5;
									ROS_INFO("LWF : P to P finished");
								}
								else
								{
									flag_turn = 55;
									ROS_DEBUG("LWF : Distance PP  is %d", dist_pp);
								}
							}
						}
						if(p<presence and done_pp == 2)
						{
							...
							if(setpoint <= 0.7000)
							{
								if(wall_ref == 0)
								{
									...
									wall_ref = 1;
									ROS_INFO("LWF : Straight before Turn started");
								}
								if(wall_ref == 1)
								{
									...
									{
										flag_turn = 3;
										ROS_DEBUG("LWF : Distance straight is %d", dist);
									}
								}
								else if(wall_ref == 2)
								{
							...
							{
								flag_turn = 8;
								ROS_INFO_THROTTLE(2, "LWF : Inside turn");
							}
						}
					}
				...
				{
					flag_turn = 12;
					ROS_WARN_THROTTLE(1, "LWF : Stopped due to right or front obstacle");
				}
			}
		}
		else if(starter != 10)
		...
		if(flag1 == 1)
		{
			if(flag_turn == 0)
			{
				...
				if(integral_end == -1.0)
                {
                    ROS_INFO("LWF : Wall following started");
                }

				float op =  lw_tuner.pid_tuner(KP, KI, KD, integral_end, err);
                ...
                float var = max(float(-100.0), min(op, float(100.0)));

                cmd.linear.x=125 - var;
                cmd.angular.z=125 + var;
                mov_cmd.publish(cmd);
...
{
	ros::init(argc, argv, "brs_left_wall");
	ros::NodeHandle n;
	
	N::debugging_next obj_lw;
	...
	while (ros::ok())
	{
		if(starter == 10)
        {
            //allowed = obj_lw.sensor_status(1.0, 0, 1, 0, 1, 1);
			allowed = 1;
            if(allowed == 0)
            {
                integral_end = -1.0;
