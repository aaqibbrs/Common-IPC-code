...
float setpoint_array[100], lg[100], err, setpoint_array_flat[100], err_flat;

int h, reference = 0, one_time = 0, flag_turn = 0, starter = 0, stepmanual = 0, flag_sf = 0;

geometry_msgs::Twist msg1;

T::tuner lw_tuner_hybrid;
...
void laser_scan_callback(const sensor_msgs::LaserScan::ConstPtr& scan) 
{
	if(allowed == 1)
	{
		...
		if(starter == 10)
		{
			...
			if(new_command == 1)
			{
				ros::param::get("/ws_setpoint",setpoint);
			...
			for(int i = 0; i < count; i++)
			{
				...
				if(first == 0)
				{
					if((degree >= -0.001 and degree <= 0.001))
					{
				...
				if(first == 1)
				{
					...
					first = 2;
					ROS_INFO("LWF : Centre value is %f", centre);
				...
				else
				{
					if(setpoint < 0.70)
					{
						...
						if(reference != 1)
						{
							if((degree>10.0 and degree<30.0) and scan->ranges[i]>0.1 and scan->intensities[i]!=0 and sf_ref == 0) 
							{   
								if(scan->ranges[i] <=short_wall)
								{
									flag_sf = 2;
						...
						{
							if((degree>10.0 and degree<30.0) and scan->ranges[i]>0.1 and scan->intensities[i]!=0) 
							{   
								if(scan->ranges[i] <=short_wall)
								{
									flag_sf = 2;
					...
					if((degree>=30.0 and degree<=89.0) and (scan->ranges[i] < (2.0 * setpoint_array[int(degree)])) and scan->ranges[i] < lg[int(degree)] and scan->intensities[i]!=0)
					{
			...
			{
				flag1=1;
				if(stopped == 0.0 and stopped_front == 0.0)
				{
					if((flag_sf == 0 or flag_sf == 2) and reference != 1)
					{
						...
						if(p<presence and done_pp != 2)
						{
							...
							if(done_pp == 1)
							{
								...
								{
									flag_turn = 55;
									ROS_INFO("RWF : Distance PP  is %d", dist_pp);
								}
							}
						}
						if(p<presence and done_pp == 2)
						{
							...
							if(setpoint <= 0.7000)
							{
								...
								if(wall_ref == 1)
								{
									...
									{
										flag_turn = 3;
										ROS_INFO("LWF : Distance straight is %d", dist);
									}
								}
								else if(wall_ref == 2)
								{
				...
				{
					flag_turn = 12;
					ROS_WARN(" LWF : Stopped due to right or front obstacle");
				}
			}
		}
		else if(starter != 10)
		...
		if(flag1 == 1)
		{
			if(flag_turn == 1)//short wall turn right
			{
...
void scan_callback(const sensor_msgs::LaserScan::ConstPtr& scan1) 
{
	if(allowed_flat == 1)
	{
		int count = scan1->scan_time / scan1->time_increment;

		if(starter == 10)
		{
			int error_cal = 0, lmn, stu, pqr;
			...
			for(int i = 0; i < count; i++)
			{
				float degree = RAD2DEG(scan1->angle_min + scan1->angle_increment * i);
				...
				if(first_flat == 1)
				{
					for(int k=40; k<=89;k++)
					{
						setpoint_array_flat[k] = setpoint/sin((k) * PI/180);
					}
					first_flat = 2;
				}
				else
				{
					if((degree>=40.0 and degree<=89.0) and (scan1->ranges[i] < (2.0 * setpoint_array[int(degree)])) and scan1->intensities[i]!=0)
					{
						if(newdegree!=degree and error_cal == 0)
						{
							lmn = int(degree);
							average_new[lmn] = scan1->ranges[i];

							first_error = setpoint_array_flat[lmn] - average_new[lmn];
							error_cal = 1;
							newdegree = degree;
							stu = lmn;
						}
						...
						{
							pqr = int(degree);
							average_new[pqr] = scan1->ranges[i];

							second_error = setpoint_array_flat[pqr] - average_new[pqr];
							error_cal = 2;
							newdegree = degree;
						}
						if(error_cal == 2)
						...
						if(error_cal == 4)
						{
							second_error = setpoint_array_flat[stu] - average_new[stu];
			...
			average = average_new[stu];
			setpoint_new = setpoint_array_flat[stu];

			err_flat=(setpoint_new-average);
			err_flat = -err_flat;
		}
	}
    else
...
{
	ros::init(argc, argv, "left_hybrid");
	ros::NodeHandle n;
	
	N::debugging_next obj_lw_slant;
	N::debugging_next obj_lw_flat;
	...
	ros::Subscriber laser_sub_flat = n.subscribe<sensor_msgs::LaserScan>("/scan", 2, scan_callback);
	...
	ros::Publisher node_work = n.advertise<std_msgs::Bool>("/lw_work", 10);
	ros::Rate rate(40);
	...
	while (ros::ok())
	{
		if(starter == 10)
        {
            allowed = obj_lw_slant.sensor_status(1.0, 0, 1, 0, 1, 1);
			allowed_flat = obj_lw_flat.sensor_status(1.0, 0, 1, 1, 0, 0);
			...
			if(flag1 == 1)
			{
				if(flag_turn == 0)
				{
					...
					float op =  lw_tuner_hybrid.pid_tuner(KP, KI, KD, integral_end, error);
