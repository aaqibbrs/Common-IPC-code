...
float safety_var = 2.0;

float KP = 0.2;
float KI = 0.0;
float KD = 0.1;

int right_now = 0, first_time = 0, left_side = 0, only_twenty = 0;
float time_set, timer_diff, ws_setpoint, lidar_end;

int enc_count = 750, presence = 8, obstacle_avoidance = 0;
...
long int first_next_right = 0, left_wheel = 0, right_wheel = 0, first_left = 0, first_pp = 0;
    ...
    if(allowed == 1)
    {
        ...
        if(starter == 30)
        {
            ...
            if(new_command == 1)
            {
                ...
                ros::param::get("/lidar_left_end", lidar_end);

                //setpoint = ws_setpoint + lidar_end;
                setpoint = ws_setpoint;

				enc_pp = int(((p_to_p/3.281)*left_enc_ppr)/CIRCU);
                
				turning_radius = setpoint*0.15*1.2;
				lidar_height = lidar_height + 0.0024;
				new_command = 0;

                ppr_d_2 = int(right_enc_ppr/10.0);
            ...
            for(int i = 30; i < count; i++)
            {
                ...
                if(first == 1)
                {
                    ...
                    for(k=10; k<=70;k++)
                    {
                        height = lidar_height + ((k-10) * 0.00024); //4.4 cm total height so, 0 to 90 is 2.2 cm, so for 1 degree = 2.2/90 cm
                    ...
                    first = 2;
                    short_wall = min(sw_param, (0.8 * centre));
                    ROS_INFO("LSF : Centre : %f and Short wall : %f", centre, short_wall);
                }
                else if(first == 2)
                {
                    if((degree>=-5.0 and degree<=25.0) and scan->ranges[i]>0.1 and scan->intensities[i]!=0) 
                    {
                    ...
                    if((degree>10.0 and degree<30.0) and scan->intensities[i]!=0 and right_now == 1) 
					{
                        if(scan->ranges[i] <=short_wall)
                        {
                            left_side = 1;
                    ...
                    if(degree>=10.0 and degree<=70.0 and only_twenty == 0 and ((setpoint_array[int(degree)]*safety_var) >= scan->ranges[i]))
                    {
                        if(lidar_change == 0)
                        {
                            if((degree>=10.0 and degree<=70.0) and scan->intensities[i]!=0) //change maximum according to setpoint. find relation?
                            {
                        ...
                        {
                            if((degree>=10.0 and degree<=50.0) and scan->intensities[i]!=0) //change maximum according to setpoint. find relation?
                            {
                                if(newdegree!=degree) //to check next float degree obtained from lidar
                                {
                                    arr[j]=scan->ranges[i];
                    ...
                    if(only_twenty == 1)
                    {
                        if((degree>=10.0 and degree<=30.0) and scan->intensities[i]!=0 and ((setpoint_array[int(degree)]*safety_var) >= scan->ranges[i])) //change maximum according to setpoint. find relation?
                        {
                            if(newdegree!=degree) //to check next float degree obtained from lidar
                            {
                                arr[j]=scan->ranges[i];
                    ...
                    if(j >= 2 and new_val == 1)
                    {
                        ...
                        {
                            second_degree = int(degree_array[j-1]);
                            
                            second_error = setpoint_array[second_degree] - arr[j-1];
            ...
            if(z > 3 and first == 2)
            {
                    ...
                    if(obstacle_avoidance == 0)
                    {
                        if(flag_sf == 1)
                        {
                            ...
                            else
                            {
                                ...
                                if(right_now == 1)
                                {
                                    //take right turn only
                                    flag_turn = 9;
                                    ROS_WARN_THROTTLE(1, "LSF : take right turn because obstacle");
                                    new_dir = 0;
                                }
                                if(h<presence and lidar_change != 2 and right_now == 0)
                                {
                                    flag_turn = 10;//sensing stopping
                                    ROS_WARN_THROTTLE(1, "LSF :  Sensing stopping while turn(straight)");
                                }
                                if(h<presence and lidar_change == 2 and right_now == 0)
                                {
                                    flag_turn = 1; //shortwall turning
                                    wall_ref = 4;
                                    ROS_WARN_THROTTLE(1, "LSF :  Turning left short wall (after straight of turn)");
                                }
                            }
                        }
                        if(flag_sf == 0 and right_now == 1)
                        {
                            if(left_side == 1)
                            {
                                //take right turn
                                flag_turn = 9;
                                ROS_INFO_THROTTLE(1, "Right turn because left side obstacle");
                                only_twenty = 1;
                                hold_twenty = 0;
                                new_dir = 0;
                            }
                            if(h<presence and left_side == 0)
                            {
                                if(first_time == 2)
                                {
                                    timer_start = ros::Time::now();
                                ...
                                if(first_time == 3)
                                {
                                    ...
                                    if(timer_diff > 2.0F)
                                    {
                                        first_time = 0;
                                        only_twenty = 1;
                                        ROS_INFO("LSF : 2 second timer over");
                                    }
                                    else
                                    {
                                ...
                                //take left turn to align
                                flag_turn = 88;
                                ROS_INFO_THROTTLE(2, "LSF : Left alignment code after turn");
                                hold_twenty = 0;
                            }
                            if(h>=presence and left_side == 0)
                            {
                        ...
                        else if(flag_sf == 0 and right_now == 0)
                        {
                            ...
                            else if(h>=presence and stringent == 1 and new_flag == 0)
                            {
                                if(dist_next<= ppr_d_2)//200 is for 400ppr, so 250 is for 500 ppr
                                {
                                    ...
                                    wall_ref = 3;
                                    lidar_change = 2;
                                    ROS_WARN_THROTTLE(1, "LSF :  step following not started due to stringentt");
                                }
                                else
                                {
                                    stringent = 0;
                                    ...
                                    wall_ref = 0;
                                    lidar_change = 0;
                                    ROS_WARN_THROTTLE(1, "LSF : step following started from stringent");
                                }
                            }
                            else if(h>=presence and new_flag == 1)
                            {
                                ...
                                new_flag = 2;
                                ROS_WARN_THROTTLE(1, "LSF :  Stopping due to unkwon object detection");
                            }
                            ...
                            if(h<presence and done_pp != 2)
                            {
                                if(done_pp == 0)
                                {
                                    first_pp = left_wheel;
                                    done_pp = 1;
                                    ROS_INFO("LSF : P to P started with P to P as %f", p_to_p);
                                }
                                if(done_pp == 1)
                                {
                                    dist_pp =  int(left_wheel - first_pp);
                                    if(dist_pp>=enc_pp)
                                    {
                                        done_pp = 2;
                                        flag_turn = 5;
                                    ...
                                    {
                                        flag_turn = 55;
                                        ROS_DEBUG_THROTTLE(2, "LSF : D PP is %d", dist_pp);
                                        new_flag = 1;
                                    }
                                }
                            }
                            if(h<presence and done_pp == 2)
                            {
                                ...
                                {
                                    flag_turn = 8;
                                    ROS_INFO_THROTTLE(2,"LSF : Inside turn");
                                }
                                else
                                {
                                    if(wall_ref == 0)
                                    {
                                        ...
                                        wall_ref = 1;
                                        ROS_INFO("LSF : Sraight before turn Started");
                                    }
                                    if(wall_ref == 1)
                                    {
                                        ...
                                        if(dist>=enc_count)//750 is for 400ppr, so 935 is for 500ppr
                                        {
                                            ...
                                            wall_ref = 2;
                                            new_flag = 0;
                                            ROS_INFO("LSF : Sraight before turn completed");
                                        }
                                        else if(dist < enc_count)
                                        {
                                            flag_turn = 3;
                                            new_flag = 1;
                                            ROS_DEBUG_THROTTLE(1, "LSF : d is %d", dist);
                                    ...
                                    if(wall_ref == 2)
                                    {
                                        first_next_right = right_wheel;
                                        wall_ref = 3;
                                        new_flag = 0;
                                    }
                                    if(wall_ref == 3)
                                    {
                                        dist_next = int(right_wheel - first_next_right);
                                        if(dist_next<=ppr_d_2)//200 is for 400ppr, so 250 is for 500 ppr
                                        {
                                            flag_turn = 4;
                                            stringent = 1;
                    ...
                    else
                    {
                        if(flag_sf == 1)
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    flag_turn = 10;//sensing stopping
                                    ROS_WARN_THROTTLE(1, "LSF :  Sensing stopping while step present");
                                }
                            ...
                            if(h<presence and done_pp == 2)
                            {
                                ...
                                else
                                {
                                    if(wall_ref == 0)
                                    {
                                        first_left = left_wheel;
                                        wall_ref = 1;
                                        ROS_INFO("LSF : Straight before Turn started");
                                    }
                                    if(wall_ref == 1)
                                    {
                                        dist = int(left_wheel - first_left);
                                        ...
                                        else if(dist < enc_count)
                                        {
                                            flag_turn = 3;
                                            new_flag = 1;
                                            ROS_DEBUG("LSF : distance is %d", dist);
                                            if(dist < ratio_enc_count)//350 is for 400ppr, so 435 is for 500ppr
                                            {
                                                lidar_change = 0;
                                            }
                ...
                {
                    flag_turn = 20;
                    ROS_WARN_THROTTLE(1, "LSF :  Stopping due to right side obstacle, press 'y' to disable");
                }
            }
        }
        else if(starter != 30)
        ...
        if(flag1 == 1)
        {
            if(flag_turn == 0)
			{
                ...
                if(integral_end == -1.0)
                {
                    ROS_INFO("LSF : Step following started");
                }
                
				float op =  ls_tuner.pid_tuner(KP, KI, KD, integral_end, err);
				...
				ROS_DEBUG("  op :%f",op);
				msg1.linear.x=thres;
				msg1.angular.z = max(float(-0.10), min(op, float(0.10)));
            ...
            else if(flag_turn == 1)//short wall turn left
            {
                ...
                ROS_DEBUG("short wall turning left");
                cmd_pub.publish(msg1);
                integral_end = -1.0;
            }
            else if(flag_turn == 2)
            ...
            else if(flag_turn == 8) //turn left for more distance
            {
            ...
            else if(flag_turn == 9)
            {
                ...
                msg1.angular.z=-0.2990; //rpm/33.438  = 0.5383 for 18
                cmd_pub.publish(msg1);
                ROS_DEBUG("Inside Right turn");
                integral_end = -1.0;
            }
            else if(flag_turn == 88)//turn left for short wall correction
            {
                if(new_dir == 0)
                {
                    dir_rad = 0.0;
                ...
                {
                    if(right_wheel > (first_next_right+40))
                    {
                ...
                msg1.linear.x=0.15;
                msg1.angular.z= min(0.12F, dir_rad);
                cmd_pub.publish(msg1);
                ROS_DEBUG("Inside left alignment drift");
                integral_end = -1.0;
            }
            ...
            else if(flag_turn == 20) //sensing stopping
            {
                ...
                msg1.angular.z=0.0;
                cmd_pub.publish(msg1);
                ROS_DEBUG("LSF : Inside sensing stopping on right side");
                integral_end = -1.0;
            }
            else
            {
...
{
	ros::init(argc, argv, "brs_left_step");
	ros::NodeHandle n;

    N::debugging_next obj_ls;
    ...
    ros::Publisher node_work = n.advertise<std_msgs::Bool>("/ls_work", 10);
	ros::Rate rate(30);
	...
	while (ros::ok())
	{
        if(starter == 30)
        {
            allowed = obj_ls.sensor_status(1.0, 0, 1, 0, 1, 1);
            if(allowed == 0)
            {
                integral_end = -1.0;
            }
            ls.data = true;
			node_work.publish(ls);
        }
		ros::spinOnce();
		rate.sleep();
	}
