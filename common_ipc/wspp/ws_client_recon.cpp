...
//std::string uri="ws://34.237.0.125:5000";
//std::string uri="ws://192.168.0.105:5000";
//std::string uri="ws://3.90.165.142:5000";//http://3.92.146.82:4100/
//std::string uri="ws://localhost:9002"
std::string uri="ws://3.92.146.82:8080";//http://3.92.146.82:4100/

ros::Timer tim1,tim2,tim3,tim4,tim5,tim6,tim7,ping,recon;

std::string Pl,Pr,RID="5cbe8fa9b4105a2c38775010";
//std::string Pl,Pr,RID="5f0416890ab70020b2cbd576|";
//std::string Pl,Pr,RID="1|";
std::string AUTH = "{\"messagetype\":\"robot\",\"data\":\""+RID+"\"}";
//std::string AUTH = "{\"messagetype\":\"robot\",\"data\":\"5f0416890ab70020b2cbd576\"}";
...
//				else if(m_message!=message_1 && m_message!=message_2 && m_message!=message_3)// && m_message!=PING_MSG)
				else
				{
//					DAT=msg->get_payload();
					N_com=1;
//					std::cout<<"Message Received: "<<m_message<<"\n";
				}
			}
		}
...
void retryCB(const ros::TimerEvent&)
{
	if(Retry==1)
	{
		if(Active!=1)
		{
//Operation after connection is establised
				if(_stat=="Open")
				{
...
void tim1CB(const ros::TimerEvent&)
{
	...
	//					std::string tmp="old_"+x.substr(4);
	//					std::cout<<x.substr(4)<<"\n";
	//					std::cout<<tmp<<"\n";
	//					endpoint.send(I,tmp);
					}
...
{
//	std::string M1="{\"messagetype\":\"Input meassage\",\"message\":\""+RID+"robdata|wt|"+"level"+"\"}";
//	std::string M2="{\"messagetype\":\"Input meassage\",\"message\":\""+RID+"robdata|bat|"+"V|A|CRG"+"\"}";
//	std::string M3="{\"messagetype\":\"Input meassage\",\"message\":\""+RID+"robdata|p|"+"cpu|ram|rom|temp"+"\"}";
//	std::string M4="{\"messagetype\":\"Input meassage\",\"message\":\""+RID+"robdata|ev|"+"moisture|temp"+"\"}";
//	std::string M5="{\"messagetype\":\"Input meassage\",\"message\":\""+RID+message5+"\"}";
//	std::string M6="{\"messagetype\":\"Input meassage\",\"message\":\""+RID+message6+"\"}";
//	std::string M7="{\"messagetype\":\"Input meassage\",\"message\":\""+RID+message7+"\"}";
//	std::string M8="{\"messagetype\":\"Input meassage\",\"message\":\""+RID+"robdata|ultra|"+"L1|L2|L3|R1|R2|R3"+"\"}";
//	std::string M9="{\"messagetype\":\"Input meassage\",\"message\":\""+RID+"robdata|gps|"+"lat|lang|set"+"\"}";
//	std::string M10="{\"messagetype\":\"Input meassage\",\"message\":\""+RID+message10+"\"}";
//	std::string M11="{\"messagetype\":\"Input meassage\",\"message\":\""+RID+"robdata|compass|"+"angle"+"\"}";
	if(init==1)
	{
		if(Active==1)
		{
...
/*			if(!file.eof() && cache==1)
			{
				...
				if(x.substr(0,3)=="  ")
				{
					std::string tmp="{\"messagetype\":\"Input meassage\",\"message\":\""+RID+"old_"+x.substr(5)+"\"}";
//					std::string tmp="old_"+x.substr(5);
//					std::cout<<x.substr(5)<<"\n";
//					std::cout<<tmp<<"\n";
//					endpoint.send(I,tmp);
				}
...
void tim7CB(const ros::TimerEvent&)
{
        ...
        //std::cout<<"init---: "<<"\n";
                ...
                //std::cout<<"active---: "<<"\n";
                        if(message10!="NULL")
                        {
                        //std::cout<<"message10---: "<<message10<<"\n";
                                endpoint.send(I,message);
                                std::cout<<"wifi msg sent: "<<message<<"\n";
                                message_6=message;
                                message10="NULL";
                                if(!file.eof() && cache==1)
                                {
...
void wificb(const geometry_msgs::Vector3::ConstPtr& a)
{
	...
	//message10="robdata|wifi|"+wi_spd+"|"+wi_sth;
}
...
//	ros::Subscriber SYS = n.subscribe<geometry_msgs::Vector3>("/system",1000,syscb);
//	ros::Subscriber WFI = n.subscribe<geometry_msgs::Vector3>("/sys_wifi",1000,wificb);
//	ros::Subscriber SPD = n.subscribe<geometry_msgs::Vector3>("/speed",1000,spdcb);
//	ros::Subscriber MOTL = n.subscribe<geometry_msgs::Vector3>("/motor_l",1000,mo_lcb);
//	ros::Subscriber MOTR = n.subscribe<geometry_msgs::Vector3>("/motor_r",1000,mo_rcb);


	ros::Publisher mov_cmd = n.advertise<std_msgs::String>("/teleop", 50);	//CMD_VEL
//	ros::Publisher heading_set_p = n.advertise<std_msgs::String>("/SET_HEADING", 10);
//	ros::Publisher heading_set_t = n.advertise<std_msgs::String>("/TURN_HEADING", 10);
//	ros::Publisher set_d=n.advertise<std_msgs::String>("/SET_DIST",10);
...
/*
			std::string str=DAT;
			bool parseSuccessful = reader.parse(str, RECV);
			if ( !parseSuccessful)
			{
